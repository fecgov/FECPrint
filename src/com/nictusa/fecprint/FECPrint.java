...
import com.nictusa.fecprint.gui.FECPrintMainWindow;
import com.nictusa.fecprint.paginate.Pagination;
import com.nictusa.fecprint.pdf.PDFConcatenate;
...
public class FECPrint {
	...
	public static void main(String[] args) 
	{
		try {
			...
			logger = LogFactory.getLog(Class.forName("com.nictusa.fecprint.FECPrint"));
			...
			//Program has arguments. No arguments implies the default run mode, which is showing the Java swing window without any view. Used while fecprint is used as a desktop application.
			if(args.length > 0)
			{
				...
				for(int i = 0; i < args.length -1 ; i++)
				{
					...
					/*else if(args[i].equals("paginate"))
					{
						paginate = true;
						showWindow = false;
					}*/					
			...
			if(fileName != null)
			{
				...
				//No UI mode. Used for running as a server process or pagination. Simply generates the pdf/page numbers and quits.
				if(showWindow == false)  
				{
						...
						if(paginate == false) //PDF generation on the server
						{
							...
							File  opDir = new File(AppProperties.getOutputDirectory() + File.separator + inputFile.getName().substring(0,inputFile.getName().lastIndexOf('.')));
							if(opDir.exists())
							{
						...
						/*else //Pagination. No pdf's are generated.Only page nos.
						{
							...
							try
							{
								...
								Pagination.getAllImageNumbers(dataBuilder.getRecordBuckets(),imagenumbers);
								File  fileImageNumbers = new File(AppProperties.getOutputDirectory() + File.separator + "imagenumbers_" + inputFile.getName().substring(0,inputFile.getName().lastIndexOf('.')) + ".txt");
								outimgno = new PrintWriter(fileImageNumbers);
								logger.info("Image numbers output file is at :" + fileImageNumbers.getAbsolutePath());
								System.out.println("Image numbers output file is at " + fileImageNumbers.getAbsolutePath());
								for(int z = 1 ; z < tot; z++)
								{
									if(imagenumbers[z] != 0)
									{
				...
				//UI mode. A Java window will pop up and show the view for given data file
				else
				{
					logger.info("UI mode");
					File inputFile = new File(fileName);
	...
	private static void usageInfo()
	{
		...
		System.out.println("	Minimum JAVA requirement for FECPrint is JAVA5 and available at: http://www.java.com/en/download   ");	
